    ...
    $("#campoPesquisa").keyup(function(){
        ...
        if($("#campoPesquisa").val() == ""){
            ...
            for(var i = 0; i < obj.length; i++){
                ...
                if(tipoUrl == 1){
                    var conteudo = '<tr id="id_registro-'+obj[i].idLicenca+'"><td style="display: none">'+ obj[i].tipo +'</td><td data-label="Licença">' + obj[i].nlicenca + '</td><td data-label="DNPM">' + obj[i].dnpm + '</td><td data-label="Data Vencimento">' + obj[i].dtaVenc + '</td><td data-label="Empresa">' + obj[i].empresa + '</td><td data-label="Status">' + circulo + '</td><td style="display: none">'+ obj[i].idLicenca +'</td><td style="display: none">'+ obj[i].idArea +'</td><td style="display: none">'+ obj[i].anexoLO +'</td><td style="display: none">'+ obj[i].anexoProrrogacao +'</td><td style="display: none">'+ obj[i].status +'</td><td style="display: none">'+obj[i].dnpm1+'</td> <td style="display: none">'+obj[i].dnpm2+'</td> <td style="display: none">'+obj[i].dnpm3+'</td> <td style="display: none">'+obj[i].dnpm4+'</td> <td style="display: none">'+obj[i].dnpm5+'</td> <td style="display: none">'+obj[i].dnpm6+'</td><td><button class="btn btnActionsPesquisar" data-toggle="modal" data-target="#editarRegistro" onclick="editarLO(this)"><i class="fas fa-edit fa-1x"></i></button><button class="btn btnActionsPesquisar" data-toggle="modal" data-target="#deletarUsuario" onclick="abrirLO(this)"><i class="fas fa-file-alt fa-1x "></i></button></td></tr>';
